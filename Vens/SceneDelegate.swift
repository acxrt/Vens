    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if let windowScene = scene as? UIWindowScene {
            ...
            if onBoardingShowed {
                ...
                UserDefaults.standard.set(true, forKey: "onBoardingShowed")
                window.rootViewController = UIHostingController(rootView: CarouselPageView(carouselData.map{ CarouselCardView(carouselCard: $0)})
                    .environmentObject(UserData()))
